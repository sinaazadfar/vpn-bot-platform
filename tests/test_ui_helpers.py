...
def test_main_menus_have_buttons() -> None:
    ...
    assert cancel_only_keyboard(scope="m", cancel_action="sellerbot_cancel").inline_keyboard
    ...
    assert admin_support_settings_menu(has_support=False).inline_keyboard
    assert admin_support_settings_menu(has_support=True).inline_keyboard
...
def test_inline_keyboards_fit_callback_limit() -> None:
    ...
    for keyboard in (
        ...
        admin_support_settings_menu(has_support=True),
